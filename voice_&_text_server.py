...
def receive_text_messages(client, text_widget):
    while True:
        try:
            ...
            text_widget.insert(tk.END, f"Server: {data}\n")
...
def send_text_response(client, text):
    ...
    print("Sending text response message...")
    client.sendall(text.encode())
    print("Text response message sent to server.")
    
def perform_action(choice, client, record_seconds=None,text=None,text_widget=None):
    ...
    elif choice == "Send Text":
        send_text_response(client, text)
    elif choice == "Receive Text":
        threading.Thread(target=receive_text_messages, args=(client, text_widget), daemon=True).start()
      

def on_action_button_click(selected_choice,text_widget=None):
    ...
    elif selected_choice == "Send Text":
        text = text_entry.get("1.0", "end-1c")
        perform_action(selected_choice, client, text=text)
    elif selected_choice == "Receive Text":
        perform_action(selected_choice, client, text_widget=text_widget)   
    else:
...
# Create GUI
root = tk.Tk()
root.title("Audio and Text Communication Server")
...
# Text Entry
text_entry_label = tk.Label(root, text="Enter text message:")
text_entry_label.pack()
text_entry = tk.Text(root, height=5, width=50)
text_entry.pack()

# Text Widget for Incoming Messages
...
text_radio = tk.Radiobutton(root, text="Send Text", variable=choice_var, value="Send Text")
text_radio.pack(anchor=tk.W)
receive_text_radio = tk.Radiobutton(root, text="Receive Text", variable=choice_var, value="Receive Text")
receive_text_radio.pack(anchor=tk.W)


# Action Button
action_button = tk.Button(root, text="Perform Action", command=lambda: on_action_button_click(choice_var.get(), text_widget=text_widget))
action_button.pack()
